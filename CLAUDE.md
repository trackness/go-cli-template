...
- **Non-interactive, always.** No prompts. Commands that mutate target
  state carry the `<cli-name>/mutating` cobra annotation on the
  **leaf** command (defined as `annotationMutating` in
  `internal/cli/root.go`). Groups do not inherit the annotation and
  should not carry it — the group dispatches; the leaf mutates. The
  root `PersistentPreRunE` rejects annotated invocations that supply
  neither `--yes` nor `--dry-run` with `CONFIRMATION_REQUIRED` /
  exit `1`, after config resolution so env/file can contribute the
  flag values. A TTY-less invocation with missing input exits the
  same way — never block waiting on stdin.
...
**Default:** redact secret values at every rendering site — config
view, error details, and resty debug curl dumps — before they reach
stdout or stderr.

- **Sensitive config keys.** Literal matches `token`, `password`,
  `secret`; suffix matches `-token`, `-secret`, `-password`, `-key`.
  Case-insensitive. `config view` replaces matching values with the
  `<redacted>` placeholder (`output.Redacted`) before serialising.
  Source attribution (`flag`, `env`, `file:<path>`, `default`) is
  preserved — skills diagnosing auth issues need to know *where* the
  secret came from, not *what* it was.
- **Sensitive HTTP headers.** `Authorization`, `Proxy-Authorization`,
  `Cookie`, `Set-Cookie`, plus any header name whose suffix matches
  the config-key suffix heuristic. Resty's logger is replaced with a
  slog adapter (`slogRestyLogger`) that routes every debug message
  through `output.RedactCurl` so curl dumps generated by
  `EnableGenerateCurlOnDebug` arrive at stderr with secrets already
  scrubbed.
- **Extension hook.** `output.SensitiveKeys`, `output.SensitiveSuffixes`,
  and `output.SensitiveHeaders` are exported `[]string` slices.
  Descendants append target-specific names:

  ```go
  output.SensitiveKeys = append(output.SensitiveKeys, "session-id")
  ```

  Appending is the supported extension; reassigning wholesale works
  but drops the defaults and is a foot-gun.
- **Constants as contract.** `output.Redacted` is `"<redacted>"` and is
